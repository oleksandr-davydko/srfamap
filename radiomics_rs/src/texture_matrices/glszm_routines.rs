...
pub fn glszm(image: ArrayViewD<u8>, omit_zeros: bool, fixed_max_size: usize, mask: Option<ArrayViewD<u8>>) -> (Array2<u16>, Array2<u32>) {
    ...
    let mut invalid_zones: HashMap<u32, bool> = HashMap::new();
        ...
        for i in 0..image.shape()[0] {
            for j in 0..image.shape()[1] {
                if mask_view[[i, j]] == 1 {
                    ...
                    if zone_id != 0 {
                        invalid_zones.insert(zone_id, true);
                    }
                }
            }
        }
    ...
    for (zone, freq) in unique_frequency.into_iter() {
        if invalid_zones.contains_key(&zone) {
            continue;
        }
